...
from homekit_hub.device_classifier import (
    classify_sensor_aids,
    expected_sensor_nodedef,
    sensor_char_bindings_for_accessory,
)
...
_DISCOVER_PRIMARY_SHORT_TIMEOUT_SEC = 6.0
_DISCOVER_PROBE_TIMEOUT_SEC = 6.0
_DISCOVER_PROGRESS_TICK_SEC = 2.0
_ZEROCONF_PROBE_PARAM_KEYS = ("zeroconf_unicast", "zeroconf_interfaces")
_LEGACY_HAP_DISCOVER_MARKER = "Zeroconf / hub diagnostic"
...
class Controller(Node):
    ...
    def __init__(self, poly, primary, address, name):
        ...
        self._discover_restart_started_at: float | None = None
        self._discover_progress_last_html: str | None = None
    ...
    def _mqtt_transport_notice_callback(self, code: int) -> None:
        ...
        if mqtt_transport_enabled(self._bridge_get_params()) and prev == MQTT_TRANSPORT_STATUS_CONNECTED:
            if code == MQTT_TRANSPORT_STATUS_NOT_CONNECTED:
                if self._planned_bridge_restart or self._discover_restart_notice_pending:
                    LOGGER.debug(
                        "MQTT transport dropped during planned bridge restart; "
                        "suppressing homekit_mqtt_transport_lost notice"
                    )
                else:
                    self._pg3_warn_and_notice(
                ...
                if not self._planned_bridge_restart:
                    self._pg3_warn_and_notice(
    ...
    def _maybe_restart_on_config_change(self) -> None:
        ...
        if network_changed:
            ...
            self._planned_bridge_restart = True
            try:
    ...
    def _on_full_restart_done(self, fut) -> None:
        ...
        try:
            ...
            if discover_restart_pending:
                self._discover_restart_notice_pending = False
                self._discover_restart_started_at = None
        ...
        if discover_restart_pending:
            ...
            self._clear_discover_bridge_restart_notice()
            elapsed = (
                time.time() - self._discover_restart_started_at
                if self._discover_restart_started_at is not None
                else None
            )
            self._discover_restart_started_at = None
            LOGGER.info(
            ...
            self._finalize_hap_discover_after_probe_restart()
    ...
    def _run_hub_bootstrap(self) -> None:
        ...
        self._clear_legacy_embedded_hap_discover_notice()
    ...
    def _set_discover_progress_notice(
        ...
        *,
        ...
        notice_html = "".join(parts)
        if notice_html == self._discover_progress_last_html:
            return
        self._discover_progress_last_html = notice_html
        self.Notices["discover_progress"] = notice_html

    def _clear_discover_progress_notice(self) -> None:
        self._discover_progress_last_html = None
        try:
    ...
    def _start_discover_progress_notice(
        ...
        *,
        ...
        def _tick(remaining: int, this_token: int) -> None:
            ...
            if remaining <= 0:
                return
            Timer(_DISCOVER_PROGRESS_TICK_SEC, lambda: _tick(remaining - _DISCOVER_PROGRESS_TICK_SEC, this_token)).start()

        if seconds > 0:
            Timer(_DISCOVER_PROGRESS_TICK_SEC, lambda: _tick(seconds - _DISCOVER_PROGRESS_TICK_SEC, token)).start()
        return token
    ...
    def _set_discover_bridge_restart_notice(self, override: dict[str, str]) -> None:
        ...
        self.Notices["discover_bridge_restart"] = (
            ...
            "This may take up to <b>1–2 minutes</b> on some hosts; on Polisy it is often much faster. "
            "<b>Bridge Status</b> will return to <b>Running</b> and the log will show "
            "<b>HomeKit Hub ready</b> when complete. "
    ...
    def _finalize_hap_discover_after_probe_restart(self) -> None:
        """Rewrite hap_discover preamble after probe auto-persist restart completes."""
        try:
        ...
        complete = (
            "<b>Bridge restart complete.</b> <b>Bridge Status</b> should show "
            "<b>Running</b> — "
        )
        if restarting in cur:
            cur = cur.replace(restarting, complete, 1)
        elif "Bridge restart complete." not in cur:
            cur = cur + (
                "<br/><b>Bridge restart complete.</b> <b>Bridge Status</b> should show "
                "<b>Running</b>; you can save pairing codes on the rows below."
            )
        self.Notices["hap_discover"] = cur

    def _clear_legacy_embedded_hap_discover_notice(self) -> None:
        """Remove pre-2.0.14 hap_discover notices that embedded zeroconf diagnostics."""
        try:
            cur = self.Notices["hap_discover"]
        except Exception:
            return
        if not isinstance(cur, str) or _LEGACY_HAP_DISCOVER_MARKER not in cur:
            return
        try:
            self.Notices.delete("hap_discover")
        except Exception:
            try:
                ...
                pass
        LOGGER.info(
            "Cleared legacy hap_discover notice (embedded zeroconf diagnostic from pre-2.0.14)"
        )
    ...
    def _persist_zeroconf_probe_params(self, override: dict[str, str]) -> None:
        ...
        self._discover_restart_started_at = time.time()
        self._clear_discover_progress_notice()
    ...
    def handler_discover(self, _data=None):
        ...
        try:
            ...
            discover_diag_pre = self._collect_zeroconf_diag()
            primary_timeout = discover_primary_timeout_sec(
                base_params,
                discover_diag_pre,
                default_sec=_DISCOVER_PRIMARY_TIMEOUT_SEC,
                shortened_sec=_DISCOVER_PRIMARY_SHORT_TIMEOUT_SEC,
            )
            primary_label = f"Primary scan ({int(primary_timeout)}s)"
            self._log_zeroconf_diag(context="DISCOVER", diag=discover_diag_pre)
            ...
            fut = asyncio.run_coroutine_threadsafe(
                self.bridge.discover_collect(primary_timeout),
                self.mainloop,
            )
            try:
    ...
    def _present_hap_discover_results(
        ...
        *,
        ...
        if not rows:
            ...
            extras = self._discover_notice_extras_html(
                attempt_history=attempt_history,
                primary_diag=primary_diag,
            )
            if extras:
        ...
        extras = self._discover_notice_extras_html(
            attempt_history=attempt_history,
            probe_auto_applied=probe_auto_applied,
            primary_diag=primary_diag,
        )
        if extras:
    ...
    _DEVICE_REFRESH_DEBOUNCE_SEC = 2.0

    def _schedule_refresh_device_generic_nodes(self, device_id: str) -> None:
        """Coalesce per-device HAP snapshots (e.g. nine sensor startups → one read)."""
        did = str(device_id or '').strip().lower()
        if not did:
            return
        token = int(self._device_refresh_tokens.get(did, 0)) + 1
        self._device_refresh_tokens[did] = token

        def _fire() -> None:
            if self._device_refresh_tokens.get(did) != token:
                return
            self.refresh_device_generic_nodes(did)

        Timer(self._DEVICE_REFRESH_DEBOUNCE_SEC, _fire).start()

    def refresh_device_generic_nodes(self, device_id: str, *, report: bool = True) -> None:
        """One HAP snapshot per device — fan out to thermostat and all sensor children."""
        did = str(device_id or '').strip().lower()
        if not did:
            return
        ...
        for node in list(self._generic_nodes.values()):
            if str(getattr(node, 'device_id', '') or '').strip().lower() != did:
                continue
            ...
            if node_def in ('HKHubThermostat', 'HKHubEcobeeThermostat'):
                ...
                if applied and report:
                    try:
                        ...
                        LOGGER.debug(
                            'reportDrivers after device snapshot failed for %s',
                            getattr(node, 'address', '?'),
                            exc_info=True,
                        )
            elif role in ('sensor', 'motion_sensor'):
    ...
    def refresh_generic_node(self, node: Any, *, report: bool = True) -> None:
        ...
        role = str(getattr(node, 'role', '') or '')
        node_def = str(getattr(node, 'id', '') or '')
        if role in ('sensor', 'motion_sensor') or node_def in (
            'HKHubThermostat',
            'HKHubEcobeeThermostat',
        ):
            self.refresh_device_generic_nodes(did, report=report)
            return
        rows = self.hub_snapshot_values(did)
        if not rows:
            return
        ...
        if applied and report:
            try:
                ...
                LOGGER.debug(
                    'reportDrivers after snapshot failed for %s',
                    getattr(node, 'address', '?'),
                    exc_info=True,
                )
    ...
    def _schedule_refresh_generic_node(self, node: Any) -> None:
        did = str(getattr(node, 'device_id', '') or '').strip().lower()
        if did:
            self._schedule_refresh_device_generic_nodes(did)
            return
    ...
    def _maybe_refresh_generic_sensor_nodes(self) -> None:
        ...
        device_ids = sorted(
            {
                str(getattr(n, 'device_id', '') or '').strip().lower()
                for n in nodes
                if str(getattr(n, 'device_id', '') or '').strip()
            }
        )
        for i, did in enumerate(device_ids):
            Timer(i * 0.5, lambda d=did: self.refresh_device_generic_nodes(d)).start()
    ...
    def _prefetch_sensor_nodes_from_accessories(
        ...
        for acc in getattr(pairing, 'accessories', None) or []:
            ...
            if not nm:
                continue
            bindings = sensor_char_bindings_for_accessory(aid, acc)
    ...
    @staticmethod
    def _sensor_node_class(node_def_id: str) -> type:
        nd = str(node_def_id or '').strip()
        if nd == 'HKHubSensorDry':
            return DrySensorNode
        if nd == 'HKHubMotionSensor':
            return MotionSensorNode
        return SensorNode
    ...
    def _ensure_sensor_node(
        ...
        *,
        ...
        if role == 'motion_sensor':
            ...
            if existing is not None:
                row = {
                    'aid': int(aid),
                    'role': 'motion_sensor',
                    'node_def_id': expected_sensor_nodedef('motion_sensor', char_bindings),
        ...
        bindings = dict(char_bindings or {})
        node_def_id = expected_sensor_nodedef(role, bindings)
        row = {
            'aid': int(aid),
        ...
        node_cls = self._sensor_node_class(node_def_id)
        node = node_cls(
            self,
            primary,
            addr,
    ...
    def _sensor_nodedef_stale(self, node: Any) -> bool:
        ...
        actual_nd = str(getattr(node, 'id', '') or '')
    ...
    def _recreate_stale_sensor_node(self, node: Any, addr: str) -> None:
        """Delete and re-add a sensor so IoX/PG3 pick up corrected nodedef/drivers."""
        addr = str(addr or '').strip()
        if not addr:
            return
        role = str(getattr(node, 'role', '') or 'sensor')
        ...
        node.id = expected_nd
        LOGGER.info(
            'Recreating sensor IoX node %s to fix stale nodedef/drivers (want %s)',
            addr,
            expected_nd,
        )
        role = str(getattr(node, 'role', '') or 'sensor')
            ...
            try:
                ...
                if node is None:
                    if node_def == 'HKHubEcobeeThermostat':
                        ...
                        node_cls = self._sensor_node_class(node_def)
                        node = node_cls(
                            self,
                            self.address,
                            addr,
